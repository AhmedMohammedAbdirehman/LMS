...
if (isset($_POST['assign'])) {
    ...
    // FIX: Define $update so the 'if' statement below works
    $update = $conn->query("UPDATE courses SET teacher_id = '$teacher_id' WHERE id = '$course_id'");

    if ($update) {
        // 2. Fetch names to make the log entry descriptive
        ...
        // 3. Log the activity with fallback session IDs
        $manager_id = $_SESSION['user_id'] ?? $_SESSION['id'] ?? 0; 
        $details = "Manager assigned teacher ($teacher_name) to course ($course_name)";
        
        if(function_exists('logActivity')) {
            logActivity($conn, $manager_id, 'ASSIGN', $details);
        }
    }
...
<body>
    ...
    <div class="assignment-card">
        ...
        <h2 data-lang="assign_teacher_title">Assign Teacher</h2>
        
        <div class="course-info">
            <label data-lang="label_selected_course">Selected Course</label>
            <strong><?php echo htmlspecialchars($course['title']); ?></strong>
        </div>
        
        <form method="POST">
            <div class="input-group">
                <label data-lang="label_select_instructor">Select an Instructor</label>
                <select name="teacher_id" required>
                    <option value="" data-lang="option_choose_list">-- Choose from List --</option>
            ...
            <button type="submit" name="assign" class="btn-assign">
                <i class="fas fa-check"></i> <span data-lang="btn_confirm_assignment">Confirm Assignment</span>
            </button>
            
            <a href="courses.php" class="cancel-link" data-lang="link_go_back">Nevermind, go back</a>
        </form>
    </div>
